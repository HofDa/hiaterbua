...
function statusBadgeClass(status: WorkSession['status']) {
  if (status === 'active') return 'bg-success-surface text-success-ink'
  if (status === 'paused') return 'bg-warning-callout text-warning-ink'
  ...
  return (
    <section className="app-panel p-5">
      <div className="flex items-center justify-between gap-3">
        ...
        <span className="text-sm font-medium text-ink-muted">{sessions.length}</span>
      </div>

      {sessions.length === 0 ? (
        <div className="mt-4 app-surface-row px-4 py-3 text-sm text-ink-muted">
        ...
        <div className="mt-4 max-h-[38rem] space-y-3 overflow-y-auto pr-1">
          {sessions.map((session) => (
            <article
              key={session.id}
              className="rounded-[1.15rem] border border-border bg-surface-raised px-4 py-4 shadow-sm"
            >
              <div className="flex items-start justify-between gap-3">
                <div>
                  <div className="font-medium text-ink">{getWorkLabel(session)}</div>
                  ...
                  {session.notes ? (
                    <div className="mt-2 text-sm text-ink-muted">{session.notes}</div>
                  ) : null}
                </div>
                ...
                <div className="flex flex-col items-end gap-2">
                  ...
                  <div className="flex flex-wrap justify-end gap-2">
                    <button
                      ...
                      className="rounded-full border border-border bg-surface-muted px-3 py-2 text-xs font-semibold text-ink-strong"
                    >
                    ...
                    <button
                      ...
                      disabled={isSaving}
                      className="rounded-full border border-error-border bg-error-surface px-3 py-2 text-xs font-semibold text-error-ink disabled:opacity-50"
                    >
                      Löschen
                    </button>
                  </div>
